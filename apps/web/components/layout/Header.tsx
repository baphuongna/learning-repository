...
export function Header() {
  ...
  const navLinks = [
    ...
    { href: '/my-documents', label: 'Tài liệu của tôi' },
    { href: '/my-news', label: 'Bài viết của tôi' },
    ...(user?.role === 'ADMIN' ? [{ href: '/admin/news', label: 'Admin' }] : []),
  ];
      ...
      <div className="container mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="md:hidden h-9 w-9 rounded-lg flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-colors"
          >
          ...
          <nav className="container mx-auto px-4 py-4 space-y-1">
            {navLinks.map((link) => (
              <Link
                ...
                className="block px-4 py-3 text-sm font-medium text-foreground hover:bg-muted/50 rounded-lg transition-colors"
              >
                {link.label}
              </Link>
            ))}
